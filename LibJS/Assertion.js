...
var Athrun = {
    ...
    assertNull : function(thingie, message) {
        ...
        this.assertTrue(thingie === null || thingie.toString() == "[object UIAElementNil]", message ? message + ": " + defMessage : defMessage);
    },
    /**
    ...
    assertNotEquals : function(actual, expect, message) {
        var msg = "实际值 # " + actual + " # 与期盼值 # " + expect + " # ";
        this.assertTrue(expect != actual, message ? message + ": " + msg : msg);
    },
    /**
    ...
    assertRegExptest : function(regExp, expression, message) {
        ...
        if(regExp.test(expression)) {
            var passMsg = expression + " 正则表达式:" + regExp + "匹配 " + (!message ? "" : message + " #校验成功");
            UIALogger.logMessage(passMsg);
        } else {
            var errorMsg = expression + " 正则表达式:" + regExp + "不匹配 " + (!message ? "" : message + " #校验失败");
    ...
    assertRegExpMatch : function(regExp, expression, message) {
        ...
        if(expression.match(regExp) != null) {
            var passMsg = expression + " 满足表达式:" + regExp + "匹配 " + (!message ? "" : message + "#校验成功");
            UIALogger.logMessage(passMsg);
        } else {
            var errorMsg = expression + " 不满足表达式" + regExp + "匹配 " + (!message ? "" : message + " #验证失败");
            UIALogger.logWarning(errorMsg);
        }
    },
    /**
    ...
    assertPopWindowTitle : function(expectTitle, message) {
        ...
        if(expectTitle == actualTitle) {
            var passMsg = "弹出窗口的实际title为 # " + actualTitle + " # 与期望的title # " + expectTitle + " # 相符," + (!message ? "" : message + " #校验成功");
            UIALogger.logMessage(passMsg);
        } else {
            var passMsg = "弹出窗口的实际title为 # " + actualTitle + " # 与期望的title # " + expectTitle + " # 不相符," + (!message ? "" : message + " #校验失败");
    ...
    assertSwitchState : function(obj, state) {
        ...
        try {
            ...
            if(value == state) {
                var msg = "Switch控件的实际状态为 # " + actualState + " # 与期望的状态  # " + state + " # 相符 #校验成功";
                UIALogger.logMessage(msg);
            } else {
                var errorMsg = "Switch控件的实际状态为 # " + actualState + " # 与期望的状态  # " + state + " # 不符 #校验失败";
